...
gulp.task('styles-admin', function() {
	return gulp.src('app/admin/scss/**/*.+(sass|scss)')
	.pipe(sass({ outputStyle: 'expanded' }).on("error", notify.onError()))
	.pipe(rename({ suffix: '.min', prefix : '' }))
	.pipe(autoprefixer(['last 15 versions']))
	.pipe(cleancss( {level: { 1: { specialComments: 0 } } })) // Opt., comment out when debugging
...
if (gulpversion == 3) {
	gulp.task('watch', ['styles', 'styles-admin', 'scripts', 'browser-sync'], function() {
		gulp.watch('app/sass/**/*.sass', ['styles']);
		gulp.watch('app/admin/scss/**/*.+(sass|scss)', ['styles-admin']);
...
if (gulpversion == 4) {
	gulp.task('watch', function() {
		...
		gulp.watch('app/admin/scss/**/*.+(sass|scss)', gulp.parallel('styles-admin'));
		gulp.watch(['libs/**/*.js', 'app/js/common.js'], gulp.parallel('scripts'));
		gulp.watch('app/*.html', gulp.parallel('code'))
		gulp.watch('app/admin/**/*.html', gulp.parallel('code'))
	});
	gulp.task('default', gulp.parallel('styles', 'styles-admin', 'scripts', 'browser-sync', 'watch'));
}
